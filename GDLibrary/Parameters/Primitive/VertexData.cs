...
namespace GDLibrary
{
    public class VertexData<T> : IVertexData where T : struct, IVertexType
    {
        ...
        {
            VertexData<T> other = obj as VertexData<T>;
            ...
            return this.Vertices.Equals(other.Vertices)
                && this.PrimitiveType.Equals(other.PrimitiveType)
                    && this.PrimitiveCount.Equals(other.PrimitiveCount);
        }

        public override int GetHashCode()
        {
            int hash = 31 + this.Vertices.GetHashCode();
            hash = hash * 43 + this.PrimitiveCount;
            hash = hash * 51 + this.PrimitiveType.GetHashCode();
            return hash;
        }
